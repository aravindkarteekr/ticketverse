...
export function makeCreateShow(showRepo: ShowRepository, theatreLookup: TheatreLookupPort) {
  return async (ownerId: string, input: CreateShowInput) => {
    ...
    if (context.ownerId !== ownerId) throw new ForbiddenError("You do not own this screen");
...
export function makeUpdateShow(showRepo: ShowRepository, theatreLookup: TheatreLookupPort) {
  return async (id: string, ownerId: string, input: UpdateShowInput) => {
    ...
    const theatreOwnerId = await theatreLookup.getTheatreOwnerId(show.theatreId);
    if (theatreOwnerId !== ownerId) throw new ForbiddenError("You do not own this show");
...
export function makeDeleteShow(showRepo: ShowRepository, theatreLookup: TheatreLookupPort) {
...
export function makeSearchShows(showRepo: ShowRepository, theatreLookup: TheatreLookupPort) {
  return async (query: {
    movieId?: string;
    city?: string;
    date?: Date;
