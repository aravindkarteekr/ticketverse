...
export function makeRequestTheatreOwner(requestRepo: TheatreOwnerRequestRepository) {
  return async function requestTheatreOwner(
    ...
    if (pending.some((r) => r.userId === userId)) {
      throw new ConflictError("You already have a pending theatre-owner request");
    }
