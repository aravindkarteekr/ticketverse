...
import { createScreenSchema, type CreateScreenInput } from "@ticketverse/schemas";
...
export function TheatreScreensPage() {
  ...
  const { register, control, handleSubmit, reset } = useForm<CreateScreenInput>({
    ...
    defaultValues: { name: "", layout: [{ row: "A", seatCount: 10, seatType: "regular" }] },
  });
  ...
  const deleteMutation = useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["screens", theatreId] }),
  });
  ...
  return (
    <Box maxWidth={700} mx="auto" mt={4} px={2}>
      ...
      <List>
        {screens?.map((screen) => (
          <ListItem
            key={screen.id}
            secondaryAction={
              <IconButton edge="end" onClick={() => deleteMutation.mutate(screen.id)}>
                <DeleteIcon />
              </IconButton>
            }
          >
            <ListItemText
              primary={screen.name}
              secondary={screen.layout.map((row) => `${row.row}×${row.seatCount} (${row.seatType})`).join(", ")}
            />
          </ListItem>
        ))}
      </List>
      ...
      <Box component="form" onSubmit={handleSubmit((input) => createMutation.mutate(input))} display="grid" gap={2}>
        <TextField label="Screen name" {...register("name")} />
        {fields.map((field, index) => (
          <Stack direction="row" spacing={1} key={field.id} alignItems="center">
            <TextField label="Row" {...register(`layout.${index}.row` as const)} sx={{ width: 80 }} />
            <TextField
              label="Seat count"
              type="number"
              {...register(`layout.${index}.seatCount` as const, { valueAsNumber: true })}
              sx={{ width: 120 }}
            />
            <TextField
              label="Seat type"
        ...
        <Button onClick={() => append({ row: "", seatCount: 10, seatType: "regular" })}>Add row</Button>
